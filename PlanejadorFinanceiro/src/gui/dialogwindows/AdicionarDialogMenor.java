...
public class AdicionarDialogMenor extends JDialog {
	...
	public AdicionarDialogMenor(String tab, WindowListener att) {
		...
		lblNewLabel.setBounds(21, 58, 56, 17);
		...
		spinnerDuracao.setModel(new SpinnerNumberModel(Integer.valueOf(2), Integer.valueOf(2), null, Integer.valueOf(1)));
		spinnerDuracao.setBounds(90, 111, 97, 20);
		panel.add(spinnerDuracao);
		
		JLabel lblNewLabel_1_1_1 = new JLabel("Duração");
		lblNewLabel_1_1_1.setBounds(9, 114, 88, 14);
	...
	void addDataDatabase() throws EmptyTextError{
		String tabela  = nome;
		...
		if (textFieldDespesa.getText().isEmpty()) {
			throw new EmptyTextError("Campo Nome não pode ser vazio");
		}
		...
		if(rdbtnOcasional.isSelected()) {
			...
			anoInicio =  Integer.parseInt((spinnerAnoOcasional.getValue().toString()));
			duracao = 1; 
			...
			anoInicio =  Integer.parseInt((spinnerAnoMensal.getValue().toString()));
